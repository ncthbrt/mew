...
pub fn mangle_inline_arg_name(
    enclosing_path: &im::Vector<PathPart>,
    parent_path: &im::Vector<PathPart>,
    ...
    for i in enclosing_path.iter() {
        enclosing_path_result.push_str(&mangle_template_args(i).replace('_', "__"));
        enclosing_path_result.push('_');
    }
    let mut parent_path_result = String::new();
    for i in parent_path.iter() {
        parent_path_result.push_str(&mangle_template_args(i).replace('_', "__"));
